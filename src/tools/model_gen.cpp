...
static const WeaponModelGen::WeaponDef s_builtinDefs[] = {
    ...
    {
        "ring", "Flat band with tall gemstone",
        {
            {{-0.12f, -0.015f, -0.12f}, {0.12f,  0.015f, 0.12f}},   // very flat wide band
            {{-0.025f, 0.015f, -0.025f}, {0.025f, 0.10f,  0.025f}},  // tall gemstone on top
        }, 2
    },
    {
        "shield", "Kite shield",
...
// ============================================================
//  Custom mesh: low-poly torus ring with gemstone
// ============================================================
static bool writeTorusRingOBJ(const char* path) {
    FILE* f = std::fopen(path, "w");
    if (!f) return false;

    // Torus parameters
    constexpr f32 R = 0.10f;      // major radius
    constexpr f32 r = 0.025f;     // tube radius
    constexpr u32 PATH_SEGS = 8;  // segments around the ring
    constexpr u32 TUBE_SEGS = 4;  // segments around the tube
    constexpr f32 PI2 = 6.2831853f;

    std::fprintf(f, "# Low-poly torus ring with gemstone\n");
    std::fprintf(f, "# Generated by WeaponModelGen (torus + gem box)\n\n");

    // Torus vertices and normals
    for (u32 i = 0; i < PATH_SEGS; i++) {
        f32 theta = PI2 * i / PATH_SEGS;
        f32 cx = R * cosf(theta);
        f32 cz = R * sinf(theta);
        for (u32 j = 0; j < TUBE_SEGS; j++) {
            f32 phi = PI2 * j / TUBE_SEGS;
            f32 nx = cosf(theta) * cosf(phi);
            f32 ny = sinf(phi);
            f32 nz = sinf(theta) * cosf(phi);
            std::fprintf(f, "v %f %f %f\n", cx + r * nx, r * sinf(phi), cz + r * nz);
        }
    }

    // Gemstone box on top (8 verts at x=R position)
    constexpr f32 gs = 0.03f;
    constexpr f32 gy = r + 0.01f;
    std::fprintf(f, "v %f %f %f\n", R - gs, gy, -gs);
    std::fprintf(f, "v %f %f %f\n", R + gs, gy, -gs);
    std::fprintf(f, "v %f %f %f\n", R + gs, gy + gs * 1.5f, -gs);
    std::fprintf(f, "v %f %f %f\n", R - gs, gy + gs * 1.5f, -gs);
    ...
    for (u32 i = 0; i < PATH_SEGS; i++) {
        ...
        for (u32 j = 0; j < TUBE_SEGS; j++) {
            ...
            f32 ny = sinf(phi);
            f32 nz = sinf(theta) * cosf(phi);
            f32 l = sqrtf(nx*nx + ny*ny + nz*nz);
            if (l > 0.001f) std::fprintf(f, "vn %f %f %f\n", nx/l, ny/l, nz/l);
            else std::fprintf(f, "vn 0 1 0\n");
        }
    }
    // Simplified normals for gem box (all up)
    for (u32 i = 0; i < 8; i++) std::fprintf(f, "vn 0 1 0\n");

    std::fprintf(f, "\nvt 0 0\nvt 1 0\nvt 1 1\nvt 0 1\n\n");

    // Torus faces
    for (u32 i = 0; i < PATH_SEGS; i++) {
        u32 ni = (i + 1) % PATH_SEGS;
        for (u32 j = 0; j < TUBE_SEGS; j++) {
            u32 nj = (j + 1) % TUBE_SEGS;
            u32 v0 = i * TUBE_SEGS + j + 1;
            u32 v1 = i * TUBE_SEGS + nj + 1;
            u32 v2 = ni * TUBE_SEGS + nj + 1;
            u32 v3 = ni * TUBE_SEGS + j + 1;
            std::fprintf(f, "f %u/1/%u %u/2/%u %u/3/%u\n", v0, v0, v1, v1, v2, v2);
            std::fprintf(f, "f %u/1/%u %u/2/%u %u/3/%u\n", v0, v0, v2, v2, v3, v3);
        }
    }

    // Gem box faces (6 faces = 12 triangles)
    u32 g = PATH_SEGS * TUBE_SEGS + 1; // 1-indexed gem start
    std::fprintf(f, "f %u/4/1 %u/3/1 %u/2/1\n", g+3, g+2, g+1);
    std::fprintf(f, "f %u/4/1 %u/2/1 %u/1/1\n", g+3, g+1, g+0);
    std::fprintf(f, "f %u/1/2 %u/2/2 %u/3/2\n", g+4, g+5, g+6);
    std::fprintf(f, "f %u/1/2 %u/3/2 %u/4/2\n", g+4, g+6, g+7);
...
bool WeaponModelGen::generateWeapon(const char* meshDir, const WeaponDef& def, bool force) {
    ...
    // Ring uses custom torus geometry instead of boxes
    if (std::strcmp(def.name, "ring") == 0) {
        return writeTorusRingOBJ(path);
    }
