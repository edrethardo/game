...
static const u16 s_iconData[][16] = {
    ...
    // 4: PISTOL — handgun silhouette, generated by tools/gen_item_icons.py (draw_pistol). Do NOT
    // hand-edit; change draw_pistol() and rerun build_assets.py. subtypeToIconIndex(PISTOL) == 4.
    { ITEM_ICON_PISTOL_ROWS },
    // 5: SMG — submachine-gun silhouette (long receiver + dropped magazine), generated by
    // tools/gen_item_icons.py (draw_smg). Do NOT hand-edit. subtypeToIconIndex(SMG) == 5.
    { ITEM_ICON_SMG_ROWS },
    // 6: CARBINE — rifle silhouette (long barrel + shoulder stock), generated by
    // tools/gen_item_icons.py (draw_carbine). Do NOT hand-edit. subtypeToIconIndex(CARBINE) == 6.
    { ITEM_ICON_CARBINE_ROWS },
    // 7: REVOLVER — revolver silhouette (cylinder bulge + barrel), generated by
    // tools/gen_item_icons.py (draw_revolver). Do NOT hand-edit. subtypeToIconIndex(REVOLVER) == 7.
    { ITEM_ICON_REVOLVER_ROWS },
    ...
    // 23: INFINITY CHAKRAM — ∞ symbol, generated by tools/gen_item_icons.py (draw_infinity).
    // Selected in drawIcon for a CHAKRAM whose def.infiniteFlight is set (the Infinity Chakram).
    { ITEM_ICON_INFINITY_ROWS },
    // 24: GOBLIN — grinning goblin head, generated by tools/gen_item_icons.py (draw_goblin).
    // Selected in drawIcon for the Mini Loot Goblin consumable (petSummon, no petEnemy) —
    // without it the pet item wore the generic RING glyph and vanished into a full backpack.
    { ITEM_ICON_GOBLIN_ROWS },
    // 25: PAW — paw print, generated by tools/gen_item_icons.py (draw_paw). Selected in
    // drawIcon for every enemy-bound pet consumable ("Mini <Enemy>", petEnemyIdx set).
    { ITEM_ICON_PAW_ROWS },
};
...
void ItemIconSystem::drawIcon(u32 screenWidth, u32 screenHeight,
    ...
    u32 iconIdx;
    if (def.petSummon) {
        // Pet consumable: its own glyph. Checked FIRST — the def's declared slot is RING
        // (loader plumbing only), and the ring glyph is exactly how the item disappeared into
        // a full backpack. The goblin jackpot keeps its face; enemy minis share the paw.
        iconIdx = (def.petEnemyIdx != 0xFF) ? 25 : 24;
    } else if (def.slot == ItemSlot::WEAPON && def.weaponSubtype != WeaponSubtype::NONE) {
        iconIdx = subtypeToIconIndex(def.weaponSubtype);
        // The Infinity Chakram uses the ∞ icon (index 23) instead of the normal chakram ring.
        if (def.weaponSubtype == WeaponSubtype::CHAKRAM && def.infiniteFlight) iconIdx = 23;
    } else {
