    ...
    for (u32 z = startZ; z < endZ; z++) {
        for (u32 x = startX; x < endX; x++) {
            ...
            if (solid) {
                ...
                // carries (up to MAX_PLATFORMS_PER_CELL) — top drawn like a floor quad at the slab
                // top, underside like a ceiling quad at the underside (skipped when the clamp left
                // no under-space — low stair steps), rim faces toward neighbours that don't cover
                // our band. A 2-story VERTICAL_HALL/arena cell has slabCount==1, so it emits exactly
                // what the old single-slab code did; a FOUR_STORY interior cell has 3.
                if (cell.flags & CELL_PLATFORM) {
                    ...
                    for (u8 s = 0; s < slabCount; ++s) {
                        ...
                        {   // rim faces for THIS slab. A rim spans the part of OUR band [undH, topH] a
                            // neighbour leaves exposed. Slabs within a cell are spaced further apart than
                            // their thickness, so at most ONE neighbour slab can overlap our band; find
                            // it and subtract it, which can leave a LOWER strip (below its underside)
                            // and/or an UPPER strip (above its top) — a stepped slab staircase exposes a
                            // thin strip on BOTH the taller side (upper) and the shorter side (lower), so
                            // both must be emitted or a stair underside develops a see-through gap.
                            // Equal-thickness slabs make only one strip non-empty per neighbour, so a
                            // shared edge still emits exactly one face; an equal-height neighbour emits
                            // none (interior edge). If NO neighbour slab overlaps — a punched drop-hole,
                            // or a neighbour with no slab at this story — the whole band is exposed
                            // hole/edge rim. Winding mirrors the riser faces above.
                            MaterialBucket* rbkt = getBucket(cell.wallMaterialId);
