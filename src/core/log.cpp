...
void Log::log(LogLevel level, const char* file, int line, const char* fmt, ...) {
    // Fast path: skip all work if no output destination (Release + no log file)
    bool consoleEnabled = true;
#if defined(NDEBUG) && (defined(_WIN32) || defined(__SWITCH__))
    consoleEnabled = false;
#endif
    if (!consoleEnabled && !s_logFile) return;
