...
**Hit feedback.** `Combat::applyDamage` classifies every hit into an `ImpactTier` (LIGHT/HEAVY/CRIT/KILL — `game/hit_feedback.h`) and fires the matching recipe from the tunable `kHitTiers` table: camera shake + blood/spark/debris/smoke particles inline (combat holds the FX pointers via `setFXTargets`) and a tier-tagged damage number (crit/kill styled). **Knockback** is applied authoritatively in `applyDamage` (size/boss-resisted, only with a `damageOrigin`) so it syncs over the network; `EnemyAI::update` yields while `Entity.knockbackTimer > 0` so the impulse is visible, and `tickTimers` decays it. Melee crits are threaded from `handleWeaponFire*` → `fireMelee(isCrit)` → `applyDamage`. **Taking damage** (`applyDamageToPlayer`) drives the red `Player.hurtVignette` (damage-scaled, + a low-HP pulse) rendered as a fullscreen overlay, plus the pre-existing camera kick / hit sound / `hitIndicators` directional arcs, and `Input::rumble`. Tune all feel from the `kHitTiers` table. Hit-stop is deferred (the `hitStopMs` field is reserved, 0 in v1). Full spec/plan: `docs/superpowers/specs/2026-05-23-hit-feedback-design.md`.
