...
### Achievements (Steamworks partner site — one-time setup)

The engine unlocks achievements by API name (`Steam::unlockAchievement`, no-op on itch builds).
Each name below must exist under **Steamworks → App 4819550 → Stats & Achievements →
Achievements** with EXACTLY this API name, then **Publish** the change set — an unpublished
achievement silently ignores `SetAchievement`:

| API name | Suggested display | Trigger (engine-side, already wired) |
|---|---|---|
| `ACH_FIRST_ITEM` | First Blood… er, Loot | First item picked up from the world (starting gear + F7 debug give don't count; guests unlock on the server-confirmed pickup) |
| `ACH_FULLY_EQUIPPED` | Dressed to Kill | All 7 equipment slots filled (polled 1 Hz; loading a fully-geared save counts) |
| `ACH_BUTCHERED` | Butchered | Killed by The Butcher (floor-5 boss). SP/host/split lanes only — a network guest's machine doesn't know its killer |

**Icons:** every achievement needs TWO images on the partner site — *Achieved* (color) and
*Unachieved* (grayed, shown pre-unlock) — both **exactly 64×64 px** JPG/PNG, opaque (the client
composites on dark backgrounds; avoid alpha). Upload per achievement in the same Achievements
tab, Save, then **Publish** — icons are part of the change set and stay invisible unpublished.
The gray variant is NOT auto-generated by Steam: supply the color icon desaturated/darkened.
Both variants for all shipped achievements are generated by `tools/gen_achievement_icons.py`
into `store/steam/achievements/` (committed) — `<API_NAME>.png` + `<API_NAME>_locked.png`,
ready to upload as-is. New achievement art goes in that tool (see the `create-achievement`
skill), never hand-drawn files.

Test with the dev `steam_appid.txt` + a logged-in Steam client; reset via the partner site's
"Clear all stats" (per test account) between runs.
